...
const Dashboard = () => {
    ...
    // ✅ MODIFIED: Now sends email directly via EmailJS
    const logAlert = async (alertType, itemName, status) => {
        if (!emailAlertsEnabled || !alertEmail || !isWithinSchedule(scheduleStartTime, scheduleEndTime)) {
            console.log(`Email alert for ${alertType} not sent: Not enabled, no email, or outside schedule.`);
            return;
        }
    
        const alertKey = `${alertType}_${itemName}`;
        ...
        const templateParams = {
            name: "Smart Home User", // You can customize this if you have a user's name
            email: alertEmail,
            title: `Smart Home Alert: ${alertType} in ${itemName}`,
            message: `Attention! A ${alertType} event has been ${status} in your ${itemName}. Please check your smart home dashboard for details.`
        };
        ...
        try {
            ...
            console.error("Failed to send email alert:", error.text);
        }
    };
    ...
    const renderStatusChip = (label, status, dangerCondition, warningCondition = null) => {
        let chipClass = "bg-success-subtle text-success";
                    ...
                    <div className={`card h-100 shadow-lg rounded-xl border-0 bg-white cute-card-bg-gradient ${homeStatusGlowClass}`}>
                        <div className="card-body p-4">
                            <h5 className="card-title text-primary mb-4 fw-bold d-flex align-items-center">
                                <span className="me-2 fs-4">🏠</span>  Home Status
                            </h5>
                ...
                <div className="col-md-6 col-lg-4">
                    <div className="card h-100 shadow-lg rounded-xl border-0 bg-white cute-card-bg-gradient-pets">
                        <div className="card-body p-4 d-flex flex-column">
                            <h5 className="card-title text-primary mb-4 fw-bold d-flex align-items-center">
                                <span className="me-2 fs-4">💖</span> Our Pets
                            </h5>
                                ...
                                <>
                                    ...
                                    <ul className="list-unstyled mb-4 flex-grow-1 overflow-auto">
                                        {Object.values(pets).map((pet) => {
                                            ...
                                            const isFoodLow = foodPercentage <= 20;
                                            const isWaterLow = waterPercentage <= (pet.waterDispenserSettings?.refillThreshold || 20);
